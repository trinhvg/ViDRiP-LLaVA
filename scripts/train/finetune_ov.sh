...
BASE_RUN_NAME="llavanext-${VISION_MODEL_VERSION_CLEAN}-Qwen2.5-7B-Instruct-mlp2x_gelu-pretrain_PathResVL_pretrain"
...
#RUN_NAME="llava-onevision-${VISION_MODEL_VERSION_CLEAN}-${LLM_VERSION_CLEAN}-ov_PathResVL_instruct_image_2dms-anyres_bs2"
#RUN_NAME="llava-onevision-${VISION_MODEL_VERSION_CLEAN}-${LLM_VERSION_CLEAN}-ov_PathResVL_instruct_video_chunk_yolo_gpt4o_train_cls33_all+2dms-anyres_bs2"
...
RUN_NAME="llava-onevision-${VISION_MODEL_VERSION_CLEAN}-${LLM_VERSION_CLEAN}-ov_PathResVL_instruct_sc_video_v0+2dms-anyres_bs2"
...
ACCELERATE_CPU_AFFINITY=1 torchrun --nproc_per_node="${NUM_GPUS}" --nnodes="${NNODES}" --node_rank="${RANK}" --master_addr="${ADDR}" --master_port="${PORT}" \
    ...
    --data_path /data1/trinh/code/ViLa/video/LLaVA-NeXT/scripts/train/onevision.yaml \
