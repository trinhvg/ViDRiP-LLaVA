...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048,
    ...
    for j, sentence in enumerate(source):
        ...
        if role == "<|im_start|>user":
            ...
            _target = [im_start] + [IGNORE_INDEX] * len(tokenizer(role).input_ids) + _input_id[len(tokenizer(
...
def eval_model(args):
    ...
    for line in tqdm(questions):
        ...
        input_ids = preprocess_qwen([line["conversations"][0], {'from': 'gpt', 'value': None}], tokenizer,
                                    has_image=True).cuda()
        ...
        if len(line["conversations"]) > 2:
            ...
            for i in range(2, len(line["conversations"]), 2):
                ...
                input_ids_new = preprocess_qwen([line["conversations"][i], {'from': 'gpt', 'value': None}], tokenizer,
                                                has_image=True).cuda()
